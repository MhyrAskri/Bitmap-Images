...
void Image::ExportRotate(const char *path) const {
    ...
    float pixel[mHeight][mWidth][3];

    for (int y = 0 ; y < mHeight ; ++y) {

        for (int x = 0 ; x < mWidth ; ++x) {

            pixel[x][y][0] = GetColor(x , y).r * 255.0f;
            pixel[x][y][1] = GetColor(x , y).g * 255.0f;
            pixel[x][y][2] = GetColor(x , y).b * 255.0f;
    ...
    for (int y = 0 ; y < mHeight ; ++y) {
        ...
        for (int x = 0 ; x < mWidth ; ++x) {
            ...
            unsigned char r = static_cast<unsigned char>(pixel[y][x][0]);
            unsigned char g = static_cast<unsigned char>(pixel[y][x][1]);
            unsigned char b = static_cast<unsigned char>(pixel[y][x][2]);
